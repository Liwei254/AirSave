...
export default function Airtime() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const result = await buyAirtime({ amount: numericAmount, phone: phone.trim(), operator, description: `${selectedOperator} airtime purchase`, goalId: goal?._id || goal?.id || null, idempotencyKey: createIdempotencyKey() });
      ...
      setPhone("");
      setSubmitted(false);
      setFeedback({ type: "success", message: roundUp.savings > 0 ? `Airtime purchased. ${formatCurrency(roundUp.savings)} was automatically saved${goal ? ` to ${goal.name}` : ""}.` : "Airtime purchased successfully." });
    } catch (error) {
