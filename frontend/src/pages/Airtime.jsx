...
export default function Airtime() {
  ...
  const canSubmit = Boolean(goal) && Boolean(phone.trim()) && numericAmount > 0 && total <= walletBalance && !submitting;
  const isLoading = profileQuery.isLoading || walletQuery.isLoading || activeGoalQuery.isLoading;

  async function submit(event) {
    event.preventDefault();
    setSubmitted(true);
    if (!goal) { setFeedback({ type: "error", message: "Create an active savings goal before buying airtime with automatic savings." }); return; }
    if (!phone.trim() || !numericAmount) { setFeedback({ type: "error", message: "Enter a phone number and airtime amount." }); return; }
    if (total > walletBalance) { setFeedback({ type: "error", message: "Insufficient wallet balance for the airtime purchase and round-up." }); return; }
    setFeedback(null);
    setSubmitting(true);
    try {
      ...
      setPhone("");
      setSubmitted(false);
      setFeedback({ type: "success", message: roundUp.savings > 0 ? `Airtime purchase confirmed. ${formatCurrency(roundUp.savings)} was automatically added to ${goal.name}.` : "Airtime purchase confirmed." });
    } catch (error) {
