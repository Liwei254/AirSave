...
import { purchaseAirtime } from "../services/postgres/prismaAirtimeService.js";
...
export async function initiatePayment(req, res, next) {
  try {
    ...
    return sendSuccess(res, { statusCode: 201, message: result.message, data: result });
  } catch (error) { return next(error); }
}

export async function purchaseAirtimeController(req, res, next) {
  try {
    const result = await purchaseAirtime(req.user._id, { ...(req.body || {}), ...(req.validatedData || {}) });
    return sendSuccess(res, { statusCode: 201, message: result.message, data: result });
  } catch (error) { return next(error); }
}
...
export async function allocateSavings(req, res, next) {
  try {
    ...
    const result = await allocateSavingsToGoal(req.user._id, payload.goalId, payload.amount, { idempotencyKey: payload.idempotencyKey, description: payload.description, type: "save" });
    return sendSuccess(res, { statusCode: 201, message: result.message, data: result });
  } catch (error) { return next(error); }
}
...
export async function handlePaymentCallback(req, res, next) {
  try {
    ...
    return sendSuccess(res, { message: result.status === "confirmed" ? "Payment confirmed and savings posted." : "Payment callback processed.", data: result });
  } catch (error) { return next(error); }
}
...
export async function getPaymentStatus(req, res, next) {
  try {
    ...
    return sendSuccess(res, { message: "Payment status fetched successfully", data: result });
  } catch (error) { return next(error); }
}
...
export async function getSavingsActivity(req, res, next) {
  try {
    ...
    return sendSuccess(res, { message: "Savings activity fetched successfully", data: activity });
  } catch (error) { return next(error); }
}
